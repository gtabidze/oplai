...
export function AppSidebar() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from("profiles").select("full_name, avatar_url").eq("id", user.id).single().then(({
        data
      }) => setProfile(data));
    }
  }, [user]);
                    ...
                    <button className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-accent transition-colors">
                      <Avatar className="h-8 w-8 flex-shrink-0">
                        ...
                        <AvatarFallback className="bg-primary/10 text-primary text-xs">
                          {profile?.full_name?.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2) || user.email?.[0].toUpperCase() || "U"}
                        </AvatarFallback>
                      </Avatar>
